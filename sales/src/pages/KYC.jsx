...
export default function KYC() {
  ...
  const showDetails = async () => {
    try {
      const response = await fetch(URI + `/admin/salespersons/get/${userid}`, {
        method: "GET",
        credentials: "include",
        headers: {
          "Content-Type": "application/json",
  ...
  return (
    <div
      className={`w-full h-screen bg-white py-8 px-4 sm:px-8 border border-[#cfcfcf33] rounded-lg`}
    >
      ...
      >
        ...
        <div className="w-full grid gap-4 place-items-center grid-cols-1 md:grid-cols-3 xl:grid-cols-4 mb-4">
          ...
          <div className="w-full">
            <label
              className={`${
                imageFiles.adharImage ? "text-green-600" : "text-[#00000066]"
              } block text-sm leading-4 font-medium`}
            >
          ...
          <div className="w-full">
            <label
              className={`${
                imageFiles.panImage ? "text-green-600" : "text-[#00000066]"
              } block text-sm leading-4 font-medium`}
            >
          ...
          <div className="w-full">
            <label
              className={`${
                imageFiles.reraImage ? "text-green-600" : "text-[#00000066]"
              } block text-sm leading-4 font-medium`}
            >
              Upload Rera Image
            </label>
            <div
              className={`${
                userData.reraimage ? "block" : "hidden"
              } relative mt-2`}
            >
              <img
                src={URI + userData?.reraimage}
                onClick={() => {
                  window.open(URI + userData?.adharimage, "_blank");
                }}
                alt="RERA Preview"
