...
export const visitScheduled = (req, res) => {
  ...
  db.query("SELECT * FROM enquirers WHERE enquirersid = ?", [Id], (err, result) => {
    ...
    db.query(insertSQL, [Id, visitDate, visitRemark, enquiryStatus, currentdate, currentdate], (err, insertResult) => {
      if (err) {
...
export const token = (req, res) => {
  ...
  db.query("SELECT * FROM enquirers WHERE enquirersid = ?", [Id], (err, result) => {
    ...
    db.query(insertSQL, [Id, paymenttype, remark, dealamount, enquiryStatus, imagePath, currentdate, currentdate], (err, insertResult) => {
      if (err) {
        console.error("Error inserting visit:", err);
        return res.status(500).json({ message: "Database error", error: err });
      }
...
export const followUp = (req, res) => {
  ...
  db.query("SELECT * FROM enquirers WHERE enquirersid = ?", [Id], (err, result) => {
    ...
    db.query(insertSQL, [Id, followUpRemark, enquiryStatus, currentdate, currentdate], (err, insertResult) => {
      if (err) {
        console.error("Error inserting visit:", err);
        return res.status(500).json({ message: "Database error", error: err });
      }
...
export const cancelled = (req, res) => {
  ...
  db.query("SELECT * FROM enquirers WHERE enquirersid = ?", [Id], (err, result) => {
    ...
    db.query(insertSQL, [Id, cancelledRemark, enquiryStatus, currentdate, currentdate], (err, insertResult) => {
      if (err) {
