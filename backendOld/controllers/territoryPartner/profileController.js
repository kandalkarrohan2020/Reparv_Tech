...
export const editProfile = (req, res) => {
  ...
  db.query("SELECT userimage FROM territorypartner WHERE id = ?", [userId], (err, result) => {
    if (err) {
...
export const changePassword = async (req, res) => {
  ...
  try {
    ...
    db.query("SELECT password FROM territorypartner WHERE id = ?", [userId], async (err, result) => {
      if (err) {
        console.error("Error fetching user:", err);
        return res.status(500).json({ message: "Database error", error: err });
      }
      ...
      db.query("UPDATE territorypartner SET password = ? WHERE id = ?", [hashedPassword, userId], (updateErr) => {
        if (updateErr) {
