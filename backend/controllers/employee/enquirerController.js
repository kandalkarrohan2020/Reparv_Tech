...
export const getAll = (req, res) => {
  ...
  let sql;
  let params = [projectPartnerId, projectPartnerId, projectPartnerId]; // default params
...
export const getProperties = (req, res) => {
  ...
  db.query(
    propertySql,
    [minBudgetValue, maxBudgetValue, category, state, city, projectPartnerId],
    (err, propertyResults) => {
      if (err) {
...
export const getPropertyList = (req, res) => {
  ...
  db.query(enquirySql, [enquiryId], (err, enquiryResults) => {
    ...
    db.query(
      propertySql,
      [minbudget, maxbudget, category, state, city, projectPartnerId],
      (err, propertyResults) => {
        if (err) {
          console.error("Error fetching properties:", err);
          return res
