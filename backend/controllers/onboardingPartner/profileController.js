...
export const editProfile = (req, res) => {
  ...
  db.query("SELECT userimage FROM onboardingpartner WHERE partnerid = ?", [userId], (err, result) => {
    if (err) {
    ...
    let updateSql = `UPDATE onboardingpartner SET fullname = ?, username = ?, contact = ?, email = ?, userimage = ?, updated_at = ? WHERE partnerid = ?`;
    const updateValues = [
      fullname,
      username,
      contact,
...
export const changePassword = async (req, res) => {
  ...
  try {
    ...
    db.query("SELECT password FROM onboardingpartner WHERE partnerid = ?", [userId], async (err, result) => {
      if (err) {
        console.error("Error fetching user:", err);
        return res.status(500).json({ message: "Database error", error: err });
      }
      ...
      db.query("UPDATE onboardingpartner SET password = ? WHERE partnerid = ?", [hashedPassword, userId], (updateErr) => {
        if (updateErr) {
