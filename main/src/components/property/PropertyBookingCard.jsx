...
const PropertyBookingCard = ({ propertyInfo }) => {
  const {priceSummery, setPriceSummery, setShowPriceSummery, setShowBenefitsPopup, setShowInquiryForm } = useAuth();
  return (
    <div
      className={`w-full max-w-[540px] flex flex-col gap-4 bg-white rounded-xl p-6 shadow border`}
    >
      {/* Title */}
      <h2 className="text-2xl font-semibold">{propertyInfo.propertyName}</h2>
      ...
      <div className="flex items-center justify-between text-gray-700 mt-3">
        <div className="flex items-center justify-center gap-1 text-base text-[#00000099] ">
      ...
      <button
        ...
        className="w-full bg-[#0BB501] text-white font-semibold py-3 mt-4 rounded-lg text-lg sm:text-2xl active:scale-95 cursor-pointer"
      >
        Book Site Visit Now
      </button>
    </div>
