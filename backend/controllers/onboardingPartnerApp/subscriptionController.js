...
export const createSubscription = async (req, res) => {
  try {
    ...
    await db.query(
      `INSERT INTO subscriptions (onboardingpartnerid, plan, amount, start_date, end_date, payment_id, status)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
...
export const getUserSubscription = (req, res) => {
  ...
  db.query(
    `SELECT * FROM subscriptions WHERE onboardingpartnerid = ? ORDER BY created_at DESC LIMIT 1`,
    [userId],
    (err, rows) => {
      if (err) {
...
export const validateRedeemCode = (req, res) => {
  ...
  db.query(
    "SELECT * FROM redeem_codes WHERE redeemCode = ? AND status = 'Active' AND planId = ?",
    [code, planid],
    (err, codes) => {
      if (err) {
