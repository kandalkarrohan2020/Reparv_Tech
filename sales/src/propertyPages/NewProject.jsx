...
export default function NewProject() {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(URI + "/frontend/newproject", {
        method: "GET",
  ...
  const fetchAllCity = async () => {
    try {
      const response = await fetch(URI + "/frontend/newproject/allcity", {
        method: "GET",
        credentials: "include", // ✅ Ensures cookies are sent
        headers: {
          "Content-Type": "application/json",
  ...
  const fetchLocation = async () => {
    try {
      const response = await fetch(URI + "/frontend/newproject/alllocation", {
        method: "GET",
        credentials: "include", // ✅ Ensures cookies are sent
        headers: {
          "Content-Type": "application/json",
      ...
      <div className="w-full flex flex-wrap gap-2 justify-between sm:px-5">
        <div className="w-full sm:w-[350px] h-10 sm:h-15 flex gap-3 items-center justify-start border border-[#00000033] rounded-lg px-4 sm:p-4 focus:outline-none">
          ...
          <input
            type="text"
            placeholder="Search..."
            className=" focus:outline-none text-sm bg-transparent sm:text-base"
          />
        </div>
