...
export const getAll = (req, res) => {
  const sql =
    "SELECT * FROM properties WHERE propertytypeid='FarmHouse' AND status='active' AND approve='Approved' AND city = ? ORDER BY propertyid DESC";
...
export const getAllCity = (req, res) => {
  const sql =
    "select distinct city from properties where status='active' and approve='Approved' and propertytypeid='FarmHouse' ";
...
export const getAllLocation = (req, res) => {
  const sql =
    "select distinct location from properties where status='active' and approve='Approved' and propertytypeid='FarmHouse' ";
  db.query(sql, (err, result) => {
    if (err) {
      console.error("Error fetching:", err);
      return res.status(500).json({ message: "Database error", error: err });
