...
export const editProfile = (req, res) => {
  ...
  db.query("SELECT userimage FROM salespersons WHERE salespersonsid = ?", [userId], (err, result) => {
    if (err) {
...
export const changePassword = async (req, res) => {
  ...
  try {
    ...
    db.query("SELECT password FROM salespersons WHERE salespersonsid = ?", [userId], async (err, result) => {
      if (err) {
        console.error("Error fetching user:", err);
        return res.status(500).json({ message: "Database error", error: err });
      }
      ...
      db.query("UPDATE salespersons SET password = ? WHERE salespersonsid = ?", [hashedPassword, userId], (updateErr) => {
        if (updateErr) {
