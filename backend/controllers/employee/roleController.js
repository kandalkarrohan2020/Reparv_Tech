...
export const add = (req, res) => {
  ...
  if (Id) {
    ...
    db.query("SELECT * FROM roles WHERE projectpartnerid = ? AND role = ?", [projectPartnerId, role], (err, result) => {
      if (err) return res.status(500).json({ message: "Database error", error: err });
      ...
      db.query(insertSQL, [projectPartnerId, role, currentdate, currentdate], (err, result) => {
        if (err) {
