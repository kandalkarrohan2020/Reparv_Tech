...
export const getById = (req, res) => {
  ...
  const sql = "SELECT * FROM calenders WHERE id = ?";
...
export const add = (req, res) => {
  ...
  db.query(sql, [name, contact, email, address, dob, department, position, salary, doj], (err, result) => {
    ...
    res.status(201).json({ message: "Calender added successfully", Id: result.insertId });
  });
};
...
export const update = (req, res) => {
  ...
  db.query("SELECT * FROM calenders WHERE id = ?", [Id], (err, result) => {
    ...
    if (result.length === 0) return res.status(404).json({ message: "Calender not found" });
