...
export default function Properties() {
  ...
  const { URI, setPriceSummery, setShowPriceSummery, propertyType, selectedCity } = useAuth();
  ...
  useEffect(() => {
    const filtered = properties.filter((item) => {
      const matchesLocation = 
        filteredLocations.length === 0 || filteredLocations.includes(item.location);
  ...
  return (
    <div className="properties w-full max-w-[1400px] flex flex-col p-4 sm:py-4 sm:px-0 mx-auto">
      {/* Search Bar */}
      <div className="w-full flex flex-wrap gap-2 justify-end py-2 sm:px-5">
        <div className="w-1/2 min-w-[350px] flex items-center justify-center relative">
      ...
      <div className=" relative w-full flex gap-5 py-4 sm:p-4">
        {/* Properties Filter */}
        <div
          className={`${filteredData.length > 0 ? "sm:flex" : "hidden"} ${
            isIntersecting ? "absolute bottom-0 " : "fixed"
          } 
          propertiesFilter overflow-y-scroll overflow-x-visible scrollbar-hide hidden 
            ...
            {filteredData.length > 0 ? (
              filteredData.map((property) => (
                <div
                  ...
                  className="group border border-[#00000033] rounded-2xl shadow-md bg-white hover:bg-[#076300] overflow-hidden"
                >
                  <img
