...
export const getAll = (req, res) => {
  const sql = "SELECT employees.*,departments.department,roles.role FROM employees INNER JOIN departments ON employees.departmentid=departments.departmentid INNER JOIN roles ON employees.roleid=roles.roleid ORDER BY employees.id DESC";
...
export const add = (req, res) => {
  ...
  const { name, uid, contact, email, address, dob, departmentid, roleid, salary, doj } = req.body;
...
export const status = (req, res) => {
  ...
  db.query("SELECT * FROM employees WHERE id = ?", [Id], (err, result) => {
    ...
    console.log(status);
    db.query("UPDATE employees SET status = ? WHERE id = ?", [status, Id], (err,result) => {
      if (err) {
        console.error("Error deleting :", err);
        return res.status(500).json({ message: "Database error", error: err });
      }
      res.status(200).json({ message: "Employee status change successfully" });
    });
  });
};
