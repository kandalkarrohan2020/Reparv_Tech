...
export default function KYC() {
  ...
  const showDetails = async () => {
    try {
      const response = await fetch(URI + `/admin/partner/get/${userid}`, {
        method: "GET",
        credentials: "include",
        headers: {
          "Content-Type": "application/json",
      ...
      if (!response.ok) throw new Error("Failed to fetch Onboarding Partner!");
      const data = await response.json();
      setUserData(data);
    } catch (err) {
      console.error("Error fetching:", err);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        `${URI}/admin/partner/edit/${userData.partnerid}`,
        {
          method: "PUT",
          credentials: "include",
          body: formData,
